        ...
        <article class="border-2 border-cyan-400 rounded-xl bg-slate-900 p-6 mb-6">
            ...
            <div class="flex gap-4 mb-4">
                ...
                <div class="flex-1">
                    ...
                    <span class="text-xs text-cyan-200/50"><?= date('d/m/Y H:i', (int)($post['data_criacao'] / 1000))?></span>
                </div>
            </div>
            ...
            <?php if (!empty($post['codigo'])): ?>
                <div class="bg-slate-800 rounded-lg border border-cyan-500/50 mb-6 overflow-hidden">
                    ...
                    <pre class="p-4 overflow-x-auto"><code
                                class="language-<?= htmlspecialchars($post['linguagem']) ?>"><?= htmlspecialchars($post['codigo']) ?></code></pre>
                </div>
            <?php endif; ?>

            <!-- IMAGEM (se houver) -->
            <?php if (!empty($post['imagem_url'])): ?>
                <img src="<?= htmlspecialchars($post['imagem_url']) ?>" alt="Post image"
                     class="rounded-lg mb-6 max-w-full">
            <?php endif; ?>
            ...
            <div class="flex gap-6 pt-4 border-t border-cyan-500/30">
                ...
                <form action="/post/vote" method="POST"
                      class="flex items-center gap-2 hover:bg-cyan-950/30 px-3 py-2 rounded transition cursor-pointer">
                    <input type="hidden" name="post_id" value="<?= $post['id'] ?>">
                    <button type="submit"
                            class="flex items-center gap-2 text-sm text-cyan-300 hover:text-cyan-200 transition">
                ...
                <!-- COMENTÁRIOS -->
                <a href="#comentarios"
                   class="flex items-center gap-2 text-sm text-cyan-300 hover:bg-cyan-950/30 px-3 py-2 rounded transition hover:text-cyan-200">
                    <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
        ...
        <div id="comentarios" class="space-y-2">
            ...
            <?php if (isset($_SESSION['id_usuario'])): ?>
                <form action="/comentario/criar" method="POST"
                      class="mb-6 bg-slate-900 border border-cyan-500/30 rounded-lg p-4">
                    ...
                    <button type="submit"
                            class="bg-cyan-700 hover:bg-cyan-600 text-white px-6 py-2 rounded font-semibold transition">
                        Comentar
                    </button>
                </form>
            <?php else: ?>
                <div class="mb-6 bg-slate-900/50 border border-cyan-500/30 rounded-lg p-4 text-center">
                    <p class="text-cyan-300"><a href="/login" class="text-cyan-400 hover:underline">Faça login</a> para
                        comentar</p>
                </div>
            <?php endif; ?>
            ...
            <div class="bg-slate-900/50 border border-cyan-500/20 rounded-lg p-4 hover:border-cyan-500/50 transition <?= $nivel > 0 ? 'mb-1' : 'mb-3' ?>">
                ...
                <div class="flex gap-3 mb-3">
                    <img src="/assets/img/user.svg" alt="Avatar"
                         class="w-8 h-8 rounded-full bg-cyan-500 p-0.5 flex-shrink-0">
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center gap-2">
                        ...
                        <span class="text-xs text-cyan-200/50"><?= date('d/m/Y H:i', (int)($comentario['data_criacao'] / 1000)) ?></span>
                    </div>
                </div>
                ...
                <div class="flex gap-4 text-xs opacity-0 group-hover:opacity-100 transition">
                    <button class="text-cyan-400 hover:text-cyan-300 flex items-center gap-1">
                        <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                            <path d="M7 14c-1.66 0-3-1.34-3-3 0-1.31.84-2.41 2-2.83V4c0-.83.67-1.5 1.5-1.5S9 3.17 9 4v4.17c1.16.42 2 1.52 2 2.83 0 1.66-1.34 3-3 3z"/>
                        </svg>
                        Upvote
                    </button>
                    <button class="text-cyan-400 hover:text-cyan-300 flex items-center gap-1"
                ...
                <?php if (isset($_SESSION['id_usuario'])): ?>
                    <div class="reply-form hidden mt-3 pt-3 border-t border-cyan-500/20">
                        <form action="/comentario/criar" method="POST" class="space-y-2">
                            ...
                            <button type="submit"
                                    class="bg-cyan-700 hover:bg-cyan-600 text-white px-4 py-1 rounded text-sm font-semibold transition">
                                Responder
                            </button>
                        </form>
                    </div>
